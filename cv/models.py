...
class Technology(models.Model):
    ...
    category = models.ForeignKey(Category, on_delete=models.CASCADE,
                                 verbose_name="Categoría", related_name="technologies", null=True, blank=True)
    order = models.IntegerField(
        default=0, verbose_name="Orden de visualización")
...
#        ordering = ['-datefinish'] Lo cambio a datestart porque permito nulls por nuevo campo is_current
    ...
    def time_worked(self):
        months = (self.datefinish.year - self.datestart.year) * \
            12 + self.datefinish.month - self.datestart.month
        years = months // 12
...
class Client(models.Model):
    client = models.ForeignKey(
        Work, on_delete=models.CASCADE, verbose_name="Empresa Cliente")
    company = models.CharField(
        blank=True, null=True, max_length=200, verbose_name="Empresa Cliente")
    image = models.ImageField(blank=True, null=True,
                              verbose_name="Imagen Cliente", upload_to="work")
