...
class SettingsTestCase(SimpleTestCase):
    ...
    # ==============================================================================
    # Tests 1 - Validación de carga de settings con entorno completo (caso feliz)
    # ==============================================================================
    ...
    def test_settings_cargan_correctamente_con_env(self):
        with mock.patch("dotenv.load_dotenv"), \
            ...
            self.assertEqual(app_settings.SECRET_KEY, 'clave-de-prueba-super-secreta')
            self.assertEqual(app_settings.ALLOWED_HOSTS, ['localhost', '127.0.0.1'])
            self.assertTrue(app_settings.DEBUG)



    # ==============================================================================
    # Tests 2 - Validación de carga de settings con entorno incompleto (sin secret key)
    # ==============================================================================
    ...
    def test_settings_lanza_error_si_falta_secret_key(self):
        ...
        with mock.patch("dotenv.load_dotenv"), \
            ...
            self.assertIn("No se puede iniciar la web sin una SECRET_KEY configurada.", str(context.exception))



    # ==============================================================================
    # Tests 3 - Validación de carga de settings con entorno incompleto (sin Supabase Secret)
    # ==============================================================================

    def test_settings_lanza_error_si_falta_supabase_secret(self):
        env_sin_supabase = self.env_valido.copy()
        del env_sin_supabase['SUPABASE_SECRET']  # Omitimos el secret de Supabase
        ...
        with mock.patch("dotenv.load_dotenv"), \
            ...
            self.assertIn("CRÍTICO: No se puede iniciar la web sin las variable Supabase secret.", str(context.exception))